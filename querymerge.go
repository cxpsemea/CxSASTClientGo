...
func (q *Query) MergeDependencies(other *Query) {
	...
	for _, qid := range other.Dependencies {
		if !slices.Contains(q.Dependencies, qid) {
			q.Dependencies = append(q.Dependencies, qid)
		}
	}

	for _, qid := range other.CustomDependencies {
		if !slices.Contains(q.CustomDependencies, qid) {
			q.CustomDependencies = append(q.CustomDependencies, qid)
		}
	}

	for _, qid := range other.UnknownCalls {
		if !slices.Contains(q.UnknownCalls, qid) {
			q.UnknownCalls = append(q.UnknownCalls, qid)
		}
	}
}
...
func (qc QueryCollection) MakeMergedTeamQuery(source *Query, destName string, teamsById *map[uint64]Team) (*Query, error) {
	...
	for {
		...
		newQuery.MergeDependencies(q)

		if q.BaseQueryID != q.QueryID {
			q = qc.GetQueryByID(q.BaseQueryID)
			if q.OwningGroup.PackageType != TEAM_QUERY {
				break
			}
		} else {
			break
	...
	if len(mergeString) <= 1 && destName == source.Name {
		//e.Logger.Tracef("No queries to merge, returning original")
		return source, nil
	}
...
func arrangecode(sourcecode string, commented bool) string {
	var xcode string = ""
	var xline string
	var xorig string = strings.TrimSpace(sourcecode)
...
func (q *QueryMerger) validate_queries_struct() (status int, message string) {
	...
	for i := range querieslist {
		...
		// Corp level queries can't be used in aggregation
		if querieslist[i].Level == CORP_QUERY {
		...
		if querieslist[i].Level == PROJECT_QUERY && i < len(querieslist)-1 {
			return STATUS_INVALID, "Project level query must be the last on the list"
		}
		// Check team sequence
		/*if querieslist[i].Level == TEAM_QUERY {
